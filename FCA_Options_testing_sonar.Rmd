...
```{r results = "asis", warning = FALSE, dpi=600, fig.height= 6.0, fig.width= 8.0}
datasetframeDecor[[1]][,Outcome] <- 1*(datasetframeDecor[[1]][,Outcome] == classNames[2])
...
```{r results = "asis", warning = FALSE, dpi=600, fig.height= 3.0, fig.width= 8.0}
...
prDecor <- predict(mNBDecor,decor_test)
...
meanROCAUC <- meanROCAUC
AllRocAUC <- meanROCAUC;
meanPCAROCAUC <- meanPCAROCAUC
...
for (i in c(1:length(datasetframeDecor)))
{
  if (i > 1)
  {
     datasetframeDecor[[i]][,Outcome] <- 1*(datasetframeDecor[[i]][,Outcome] == classNames[2])
  }
  mNBDecor <- filteredFit(paste(Outcome,"~."),
                   datasetframeDecor[[i]],
                    fitmethod=NAIVE_BAYES,
