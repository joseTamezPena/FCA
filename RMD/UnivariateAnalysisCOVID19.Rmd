...
```{r results = "asis"}
DEdataframe <- GDSTMDecorrelation(dataframe,Outcome=outcome,thr=0.90,verbose = TRUE,skipRelaxed=FALSE,corRank=FALSE)
#DEdataframe <- GDSTMDecorrelation(dataframe,thr=0.90,verbose = TRUE,skipRelaxed=FALSE,corRank=FALSE)
...
fcout <- round(fraction*nrow(dataframe)/10+1.0)
...
cvRawW <- randomCV(dataframe,
                outcome,
                fittingFunction= filteredFit,
                trainSampleSets= cvRaw$trainSamplesSets,
                fitmethod=  glm,
                filtermethod=univariate_Wilcoxon,
                filtermethod.control=list(pvalue=1.0,limit= fcout,thr=0.9999),
                family="binomial"
)
bprawW <- predictionStats_binary(cvRawW$medianTest,"RAW",cex=0.75)
pander::pander(bprawW$CM.analysis$tab)
pander::pander(bprawW$accc)
pander::pander(bprawW$aucs)
pander::pander(bprawW$berror)
...
cvDeW <- randomCV(DEdataframe,
                outcome,
                fittingFunction= filteredFit,
                trainSampleSets= cvRaw$trainSamplesSets,
                fitmethod=  glm,
                filtermethod=univariate_Wilcoxon,
                filtermethod.control=list(pvalue=0.20,limit= fcout,thr=0.9999),
                family="binomial"
)
bpDecorW <- predictionStats_binary(cvDeW$medianTest,"Decorrelated",cex=0.75)
pander::pander(bpDecorW$CM.analysis$tab)
pander::pander(bpDecorW$accc)
pander::pander(bpDecorW$aucs)
pander::pander(bpDecorW$berror)
...
### Here we compute the probability that the outcome-driven decorrelation ROC is superior to the RAW ROC. 
pander::pander(roc.test(bpDecorW$ROC.analysis$roc.predictor,bprawW$ROC.analysis$roc.predictor,alternative = "greater"))
