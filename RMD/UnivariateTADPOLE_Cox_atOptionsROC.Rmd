...
mlKNN <- KNN_method(formula(paste(outcome,"~.")),dataframe[,c(outcome,topSigRaw)])
pbKNN <- predictionStats_binary(cbind(dataframeTest[,outcome],predict(mlKNN,dataframeTest[,c(outcome,topSigRaw)])),"Raw KNN:ROC",cex=0.8)
...
#mlNB <- NAIVE_BAYES(formula(paste(outcome,"~.")),DEdataframeTrain[,c(outcome,topvarDe)],pca=FALSE)
#pbNB <- predictionStats_binary(cbind(dataframeTest[,outcome],predict(mlNB,DEdataframe[,c(outcome,topvarDe)])),"De NB:ROC",cex=0.8)
