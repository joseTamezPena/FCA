...
deThermal <- GDSTMDecorrelation(SalivaThermal_Avg,Outcome="class",thr=0.90,verbose = TRUE,skipRelaxed=FALSE)
...
legend("topright",legend=c("Cases", "Controls"),
       col=c("red", "blue"), lty=1:1,lwd=c(1,3), cex=0.8)
...
bpdiff <- plot(wavenumber,effectSize,las=2,cex.axis=0.5,type="l",
                                  ylab="Effect Size (Z)",
                   xlab=expression("Wavenumber (cm)"^"-1"))
...
bpdiff <- plot(x=wavenumber,y=effectSize,las=2,cex.axis=0.5,type="l",
                                  ylab="Effect Size (Z)",
                   xlab=expression("Wavenumber (cm)"^"-1"))
...
bpDecor <- predictionStats_binary(cbind(deThermal$class,predict(bagg$bagged.model,deThermalS$scaledData)),"BSWiMS Outcome-Driven GDSTM",cex=0.90)
par(op)
