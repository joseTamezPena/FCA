...
This document describes the use of the **FRESA.CAD::GDSTMDecorrelation()** function that runs the feature correlation analysis (**FDeA**) algorithm on the mfeat data set:
...
-   The feature name will be unaltered, if their maximum correlation to other features was lower than the required threshold.

-   The feature name will have the "Ba\_" prefix, if the feature was correlated but used as unaltered basis

-   The feature name will have the "La\_" prefix, if the feature was decorrelated by the algorithm. These features are discovered latent variables
...
#### The Unaltered Basis
```{r results = "asis"}
## The unaltered features set 2. 
## Continuous features that are correlated but left unaltered by the FDeA
pander::pander(colnames(decorrelatedDF)[str_detect(colnames(decorrelatedDF),"Ba_")])
```
...
featnames <- attr(decorrelatedDF,"varincluded")
