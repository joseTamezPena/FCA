...
Sakdipat Ontoum. (2022). <i>Saliva Testing Dataset</i> [Data set]. Kaggle. https://doi.org/10.34740/KAGGLE/DSV/4127956
...
COVID_19_MS <- read.csv("~/GitHub/FCA/Data/COVID-19_MS_dataset_train.csv", stringsAsFactors=TRUE)

rownames(COVID_19_MS) <- paste(COVID_19_MS$Person_ID,
                                             COVID_19_MS$Sample_ID,sep="_")

COVID_19_MS$Person_ID <- NULL
COVID_19_MS$Sample_ID <- NULL
##The fraction of samples in the training set
pander::pander(table(COVID_19_MS$PCR_result))
COVID_19_MS$PCR_result <- 1*(COVID_19_MS$PCR_result =="pos")
pander::pander(table(COVID_19_MS$PCR_result))
trainFraction=0.65
...
set.seed(2)
caseSet <- subset(COVID_19_MS, PCR_result == 1)
controlSet <- subset(COVID_19_MS, PCR_result == 0)
...
pander::pander(table(trainSet$PCR_result))
pander::pander(table(testSet$PCR_result))
...
## The Raw validation
cvBSWiMSRaw <- randomCV(COVID_19_MS,
                "PCR_result",
...
cvBSWiMSDeCor <- randomCV(COVID_19_MS,
                "PCR_result",
...
cvBSWiMSDeCorU <- randomCV(COVID_19_MS,
                "PCR_result",
                trainSampleSets= cvBSWiMSRaw$trainSamplesSets,
                fittingFunction= filteredFit,
                fitmethod=BSWiMS.model,
...
bm <- BSWiMS.model(PCR_result~.,trainSet,NumberofRepeats = 5)
bpraw <- predictionStats_binary(cbind(testSet$PCR_result,predict(bm,testSet)),"BSWiMS RAW",cex=0.60)

bmd <- BSWiMS.model(PCR_result~.,deTrain,NumberofRepeats = 5)
bpdecor <- predictionStats_binary(cbind(deTest$PCR_result,predict(bmd,deTest)),"Outcome-Driven Decorrelation",cex=0.60)

## Comparing the two ROC curves
pander::pander(roc.test(bpdecor$ROC.analysis$roc.predictor,bpraw$ROC.analysis$roc.predictor,alternative = "greater"))
## Comparing the test accuracy
testRaw <- (testSet$PCR_result-predict(bm,testSet))<0.5
testDecor <- (deTest$PCR_result-predict(bmd,deTest))<0.5
pander::pander(mcnemar.test(testRaw,testDecor))


bmdU <- BSWiMS.model(PCR_result~.,deTrainU,NumberofRepeats = 5)
bpdecorU <- predictionStats_binary(cbind(deTest$PCR_result,predict(bmdU,deTestU)),"Blind Decorrelation",cex=0.60)

## Comparing the test curves
pander::pander(roc.test(bpdecorU$ROC.analysis$roc.predictor,bpraw$ROC.analysis$roc.predictor,alternative = "greater"))
## Comparing the accuracy
testDecorU <- (deTestU$PCR_result-predict(bmdU,deTestU))<0.5
pander::pander(mcnemar.test(testRaw,testDecorU))
